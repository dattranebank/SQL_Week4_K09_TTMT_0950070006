...
/* Câu 5: Hãy tạo hàm đưa ra danh sách các hãng sản xuất có địa chỉ nhập vào từ bàn phím
(Lưu ý - Dùng hàm like để lọc) */ 
Drop function Cau5_Function
Create function Cau5_Function (@DiaChi nvarchar(30))
returns table as 
		return (Select *
				From T2_HangSanXuat T2
				Where T2.DiaChi=@DiaChi)

Select *
From Cau5_Function('Viet Nam')

/* Câu 6: Hãy viết hàm đưa ra danh sách các sản phẩm và hãng sản xuất tương ứng đã được xuất
từ năm x đến năm y với x,y được nhập từ bàn phím*/
Drop function Cau6_Function
Create function Cau6_Function (@x smalldatetime, @y smalldatetime)
returns table as 
		return (Select T1.MaSanPham, T1.TenSanPham, T2.TenHang, T1.SoLuong, T1.MauSac, T1.GiaBan, T1.DonViTinh, T1.MoTa
				From T1_SanPham T1, T2_HangSanXuat T2, T5_Xuat T5
				Where T1.MaHangSanXuat=T2.MaHangSanXuat and T5.MaSanPham=T1.MaSanPham
				and @x<T5.NgayXuat and T5.NgayXuat<@y)

Select *
From Cau6_Function('2015/1/1','2022/1/1')
...
as
	...
	if @DieuKien=0
		begin
		Insert into @atable 
		Select T1.MaSanPham, T2.TenHang, T1.TenSanPham, T1.SoLuong, T1.MauSac, T1.GiaBan, T1.DonViTinh, T1.MoTa
		From T1_SanPham T1, T2_HangSanXuat T2, T4_Nhap T4
		Where (T1.MaHangSanXuat=T2.MaHangSanXuat and T2.TenHang=@TenHangSanXuat	and T1.MaSanPham=T4.MaSanPham and T1.SoLuong=0) 
		end
	else if @DieuKien=1
		begin
		Insert into @atable 
		Select T1.MaSanPham, T2.TenHang, T1.TenSanPham, T1.SoLuong, T1.MauSac, T1.GiaBan, T1.DonViTinh, T1.MoTa
		From T1_SanPham T1, T2_HangSanXuat T2, T5_Xuat T5
		Where (T1.MaHangSanXuat=T2.MaHangSanXuat and T2.TenHang=@TenHangSanXuat	and T1.MaSanPham=T5.MaSanPham and T1.SoLuong=0) 
		end
	return
	end

Select *
From Cau7_Function('Samsung',1)
...
/* Tuần 4 Lab 7: Tạo thủ tục Stored procedure */
/* Câu 1: Tạo thủ tục nhập liệu cho bảng Hangsx với các tham biến truyền vào mahangsx, tenhang, diachi,
sodt, email. Hãy kiểm tra xem tenhang đã tồn tại trước đó hay chưa? Nếu có rồi thì không cho nhập
và đưa ra thông báo. */
Drop procedure Cau1_Procedure
Create procedure Cau1_Procedure(@MaHangSanXuat nchar(10), @TenHang nvarchar(20), @DiaChi nvarchar(30), @SoDienThoai nvarchar(20), @Email nvarchar(30))
as
	begin
	if (exists(Select * From T2_HangSanXuat T2 Where T2.TenHang=@TenHang))
		print N'Tên hãng đã tồn tại, mời bạn nhập tên khác'
	else 
		Insert into T2_HangSanXuat
		Values (@MaHangSanXuat,@TenHang,@DiaChi,@SoDienThoai,@Email)
	end

Exec Cau1_Procedure 'H05','Samsung','Korea','011-08271717','ss@gmail.com.kr'

/* Câu 2: Tạo thủ tục nhập dữ liệu cho bảng sản phẩm với các tham biến truyền vào masp, tenhangsx, 
tensp, soluong, mausac, giaban, donvitinh, mota. Hãy kiểm tra xem nếu masp đã tồn tại thì cập nhật
thông tin sản phẩm theo mã, ngược lại thêm mới sản phẩm vào bảng sanpham. */
Drop procedure Cau2_Procedure
Create procedure Cau2_Procedure(@MaSanPham nchar(10), @TenHang nvarchar(20), @TenSanPham nvarchar(20), @SoLuong int, @MauSac nvarchar(20), @GiaBan money, @DonViTinh nchar(10), @MoTa nvarchar(max))
as
	begin
	if (exists(Select * From T1_SanPham T1 Where T1.MaSanPham=@MaSanPham))
		Update T1_SanPham
		set MaHangSanXuat= (Select T2.MaHangSanXuat From T2_HangSanXuat T2 Where T2.TenHang=@TenHang),
		TenSanPham=@TenSanPham, SoLuong=@SoLuong, MauSac=@MauSac, GiaBan=@GiaBan, DonViTinh=@DonViTinh, MoTa=@MoTa
		Where T1_SanPham.MaSanPham=@MaSanPham
	else 
		Insert into T1_SanPham
		Values (@MaSanPham, (Select T2.MaHangSanXuat From T2_HangSanXuat T2 Where T2.TenHang=@TenHang), @TenSanPham, @SoLuong, @MauSac, @GiaBan, @DonViTinh, @MoTa)
	end

Exec Cau2_Procedure 'SP07','Samsung','F2 Plus','500','Luc','99999999','Chiec','Hang Cao Cap'

/* Câu 3: Viết thủ tục xóa dữ liệu bảng hangsx với tham biến là tenhang. Nếu tenhang chưa có thì thông báo,
ngược lại xóa hangsx với hãng bị xóa là tenhang (Lưu ý: xóa hangsx thì phải xóa các sản phẩm mà hangsx
này cung ứng). */
Drop procedure Cau3_Procedure
Create procedure Cau3_Procedure(@TenHang nvarchar(20))
as
	begin
	if (exists(Select * From T2_HangSanXuat T2 Where T2.TenHang=@TenHang))
		Delete from T2_HangSanXuat
		Where T2_HangSanXuat.TenHang=@TenHang
	else 
		print N'Tên hãng không có trong bảng Hãng sản xuất'
	end

Exec Cau3_Procedure 'Samsung'

/* Câu 4: Viết thủ tục nhập dữ liệu cho bảng nhân viên với các tham biến manv, tennv, gioitinh, diachi,
sodt, email, phong và 1 biến cờ Flag. Nếu Flag=0 thì cập nhật dữ liệu cho bảng nhân viên theo manv,
ngược lại thêm mới nhân viên này. */
Drop procedure Cau4_Procedure
Create procedure Cau4_Procedure(@MaNhanVien nchar(10), @TenNhanVien nvarchar(20), @GioiTinh nchar(10), @DiaChi nvarchar(30), @SoDienThoai nvarchar(20), @Email nvarchar(30), @Phong nvarchar(30),@Flag tinyint)
as
	begin
	if @Flag=0
		Update T3_NhanVien
		set TenNhanVien=@TenNhanVien, GioiTinh=@GioiTinh, DiaChi=@DiaChi, SoDienThoai=@SoDienThoai, Email=@Email, Phong=@Phong
		Where MaNhanVien=@MaNhanVien
	else
		Insert into T3_NhanVien
		Values (@MaNhanVien,@TenNhanVien,@GioiTinh,@DiaChi,@SoDienThoai,@Email,@Phong)
	end

Exec Cau4_Procedure 'NV04','asd','Nam','TPHCM','077469','asd@asd','Vat tu',1

/* Câu 5: Viết thủ tục nhập dữ liệu cho bảng Nhap với các tham biến sohdn, masp, manv, ngaynhap, soluongN,
dongiaN. Hãy kiểm tra xem masp có tồn tại trong bảng Sanpham hay không? manv có tồn tại trong bảng nhanvien
hay không? Nếu không thì thông báo, ngược lại thì hãy kiểm tra: Nếu sohdn đã tồn tại thì cập nhật bảng Nhap
theo sohdn, ngược lại thêm mới bảng Nhap. */
Drop procedure Cau5_Procedure
Create procedure Cau5_Procedure(@SoHoaDonNhap nchar(10), @MaSanPham nchar(10), @MaNhanVien nchar(10), @NgayNhap date, @SoLuongNhap int, @DonGiaNhap money)
as
	begin
	if (exists(Select * From T1_SanPham T1 Where T1.MaSanPham=@MaSanPham) and exists(Select * From T3_NhanVien T3 Where T3.MaNhanVien=@MaNhanVien))
		if (exists(Select * From T4_Nhap T4 Where T4.SoHoaDonNhap=@SoHoaDonNhap))
			Update T4_Nhap
			set MaSanPham=@MaSanPham, MaNhanVien=@MaNhanVien, @NgayNhap=NgayNhap, SoLuongNhap=@SoLuongNhap, DonGiaNhap=@DonGiaNhap
			Where SoHoaDonNhap=@SoHoaDonNhap
		else
			Insert into T4_Nhap
			Values (@SoHoaDonNhap,@MaSanPham,@MaNhanVien,@NgayNhap,@SoLuongNhap,@DonGiaNhap)
	else 
		print N'Mã sản phẩm, mã nhân viên không tồn tại trong bảng Sản phẩm, bảng Nhân viên'
	end

Exec Cau5_Procedure 'N06','SP03','NV03','2020/12/12','100','999999'

/* Câu 6:Viết thủ tục nhập dữ liệu cho bảng xuat với các tham biến sohdx, masp, nv, ngayxuat, soluongX.
Kiểm tra xem masp có tồn tại trong bảng Sanpham hay không? manv có tồn tại trong bảng nhanvien hay không?
soluongX <= Soluong? Nếu không thì thông báo, ngược lại thì hãy kiểm tra: Nếu sohdx đã tồn tại thì cập nhật
bảng Xuat theo sohdx, ngược lại thêm mới bảng Xuat*/
Drop procedure Cau6_Procedure
Create procedure Cau6_Procedure(@SoHoaDonXuat nchar(10), @MaSanPham nchar(10), @MaNhanVien nchar(10), @NgayXuat date, @SoLuongXuat int)
as
	begin
	if (exists(Select * From T1_SanPham T1 Where T1.MaSanPham=@MaSanPham) and exists(Select * From T3_NhanVien T3 Where T3.MaNhanVien=@MaNhanVien)
		and (exists(Select * From T1_SanPham T1, T5_Xuat T5 Where T1.MaSanPham=T5.MaSanPham and T1.SoLuong>=T5.SoLuongXuat)))
		if (exists(Select * From T5_Xuat T5 Where T5.SoHoaDonXuat=@SoHoaDonXuat))
			Update T5_Xuat
			set MaSanPham=@MaSanPham, MaNhanVien=@MaNhanVien, @NgayXuat=NgayXuat, SoLuongXuat=@SoLuongXuat
			Where SoHoaDonXuat=@SoHoaDonXuat
		else
			Insert into T5_Xuat
			Values (@SoHoaDonXuat,@MaSanPham,@MaNhanVien,@NgayXuat,@SoLuongXuat)
	else 
		print N'Mã sản phẩm, mã nhân viên không tồn tại trong bảng Sản phẩm, bảng Nhân viên'
	end

Exec Cau6_Procedure 'X07','SP03','NV03','2020/12/12','10'

/* Câu 7: Viết thủ tục xóa dữ liệu bảng nhanvien với tham biến là manv. Nếu manv chưa có thì thông báo,
ngược lại thì xóa nhanvien bị xóa là nv. (Lưu ý: xóa nhanvien thì phải xóa các bảng Nhap, Xuat mà
nhân viên này tham gia */
Drop procedure Cau7_Procedure
Create procedure Cau7_Procedure(@MaNhanVien nchar(10))
as
	begin
	if (exists(Select * From T3_NhanVien T3 Where T3.MaNhanVien=@MaNhanVien))
		Delete from T3_NhanVien
		Where T3_NhanVien.MaNhanVien=@MaNhanVien
	else 
		print N'Mã nhân viên không tồn tại trong bảng nhân viên'
	end

Exec Cau7_Procedure 'NV03'

/* Câu 8: Viết thủ tục xóa dữ liệu bảng sanpham với tham biến là masp. Nếu masp chưa có thì thông báo, 
ngược lại xóa sanpham với sanpham bị xóa là masp. (Lưu ý: xóa sanpham thì phải xóa các bảng Nhap, Xuat
mà sanpham này cung ứng). */
Drop procedure Cau8_Procedure
Create procedure Cau8_Procedure(@MaSanPham nchar(10))
as
	begin
	if (exists(Select * From T1_SanPham T1 Where T1.MaSanPham=@MaSanPham))
		Delete from T1_SanPham
		Where T1_SanPham.MaSanPham=@MaSanPham
	else 
		print N'Mã sản phẩm không tồn tại trong bảng Sản phẩm'
	end

Exec Cau8_Procedure 'SP06'
